...
class LogInViewTestCase(TestCase, LogInTester, MenuTesterMixin):
    ...
    def test_get_log_in_redirects_when_logged_in(self):
        ...
        redirect_url = reverse('student_dashboard')  # Correct redirect
        self.assertRedirects(response, redirect_url, status_code=302, target_status_code=200)
        self.assertTemplateUsed(response, 'student_dashboard.html')  # Correct template
    ...
    def test_succesful_log_in(self):
        ...
        response_url = reverse('student_dashboard')  # Correct redirect
        self.assertRedirects(response, response_url, status_code=302, target_status_code=200)
        self.assertTemplateUsed(response, 'student_dashboard.html')  # Correct template


    def test_succesful_log_in_with_redirect(self):
        redirect_url = reverse('profile')  # Correct expected URL
        form_input = {'username': '@johndoe', 'password': 'Password123', 'next': redirect_url}
        response = self.client.post(self.url, form_input, follow=True)
        self.assertTrue(self._is_logged_in())

        # Correct expected redirect after login
        expected_redirect_url = reverse('student_dashboard')  # Fix the expected URL
